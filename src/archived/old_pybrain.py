...
#     df = df.drop(to_del)
    ...
    for i in range(11):
        ...
        if len(this_data)>=400:
            ...
            trn_this = this_data[:-30]
            tst_this = this_data[-30:]    
...
#     training(trn_data_descs, trn_data_target)
    predictedValue = predicting('../net/10c_903chem_76d_Oct3.xml', trn_data_all, tst_data_descs)
    ...
    this_report = classification_report(tst_data_target, predictedValue)
    this_conf = confusion_matrix(tst_data_target, predictedValue)
